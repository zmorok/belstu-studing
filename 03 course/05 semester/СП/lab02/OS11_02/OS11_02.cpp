...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		logger("INFO", "Попытка открыть хэш-таблицу: HT.ht");
		...
		while (true)
		{
			...
			cout << "----------------------------------------" << endl;
			//cout << "random_device generated number: " << rand_device() << "\nmt19937: " << rand() << endl;
			...
			logger("INFO", "Генерация ключа: " + key);
			...
			if (!HT::Insert(HT, el1))
			{
				...
				logger("ERROR", "Не удалось вставить элемент с ключом " + key);
			}
			else if ((el = HT::Get(HT, el1)) == NULL)
			{
				print_error(HT::GetLastErrorHt(HT));
				logger("ERROR", "Не удалось получить элемент после вставки: " + key);
			}
			else if (el)
			{
				logger("INFO", "Элемент успешно вставлен и получен: " + key);
				HT::print(el);
			}
