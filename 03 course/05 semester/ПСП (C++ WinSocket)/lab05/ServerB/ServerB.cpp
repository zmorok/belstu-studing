...
int main()
{
    ...
    try
    {
        ...
        if (gethostname(server_hostname, sizeof(server_hostname)) == SOCKET_ERROR)
        { cout << "[SERVER] gethostname сервера ошибка: " << WSAGetLastError() << endl; return 1; }
        cout << "[SERVER] server_hostname: " << server_hostname << endl;
        ...
        while (true)
        {
            ...
            {
                hostent* host = gethostbyaddr((char*)&from.sin_addr, sizeof(from.sin_addr), AF_INET);
				string hostname = host ? host->h_name : "<unknown>";

                string msg = "\nHostname = " + hostname + '\n';
                msg += "# Address = " + (string)inet_ntoa(from.sin_addr) + ":" + to_string(ntohs(from.sin_port)) + '\n';
                ...
                cout << " [income_echo:\'" + (string)income_echo +"\']No clients requests\n";
            }
        }
    }
