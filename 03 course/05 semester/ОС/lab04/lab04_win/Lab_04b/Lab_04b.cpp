...
int main()
{
    ...
    for (int i = 0; i < mainIterations; ++i) {
        ...
        if (iter == 20) {   // — приостановка первого потока
            printf(">>> [main] SuspendThread: первый поток на итерации %d\n", iter);
            DWORD rc = SuspendThread(threads[0]);
            if (rc == (DWORD)-1) printf("SuspendThread(threads[0]) error: %lu\n", GetLastError());
        }

        if (iter == 60) {   // — возобновление первого потока
            printf(">>> [main] ResumeThread: первый поток на итерации %d\n", iter);
            DWORD rc = ResumeThread(threads[0]);
            if (rc == (DWORD)-1) printf("ResumeThread(threads[0]) error: %lu\n", GetLastError());
        }

        if (iter == 40) {   // — приостановка второго потока
            printf(">>> [main] SuspendThread: второй поток на итерации %d\n", iter);
            DWORD rc = SuspendThread(threads[1]);
            if (rc == (DWORD)-1) printf("SuspendThread(threads[1]) error: %lu\n", GetLastError());
        }
    ...
    printf(">>> [main] ResumeThread: второй поток после завершения цикла\n");
    DWORD rc2 = ResumeThread(threads[1]);
    if (rc2 == (DWORD)-1) printf("ResumeThread(threads[1]) error: %lu\n", GetLastError());
